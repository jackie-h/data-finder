...
_MAPPING_FILE = os.path.normpath(
    os.path.join(os.path.dirname(__file__), "..", "..", "datafinder_examples", "src", "datafinder_examples", "positions_mapping.md")
)
...
class TestDateExtract:
    ...
    def test_year_extracts_correct_value(self, PositionFinder):
        ...
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().year()],
    ...
    def test_month_extracts_correct_value(self, PositionFinder):
        date = datetime.date(2024, 3, 15)
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().month()],
            PositionFinder.id_().eq(1),
        ).to_pandas()
    ...
    def test_day_extracts_correct_value(self, PositionFinder):
        date = datetime.date(2024, 3, 15)
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().day()],
            PositionFinder.id_().eq(1),
        ).to_pandas()
    ...
    def test_quarter_extracts_correct_value(self, PositionFinder):
        ...
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().quarter()],
    ...
    def test_week_extracts_correctly(self, PositionFinder):
        date = datetime.date(2024, 3, 15)
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().week()],
            PositionFinder.id_().eq(1),
        ).to_pandas()
    ...
    def test_multiple_extract_columns(self, PositionFinder):
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().year(), PositionFinder.trade_date().month()],
        ).to_pandas()
...
class TestDateArithmetic:
    ...
    def test_add_days_shifts_date_forward(self, PositionFinder):
        ...
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().add_days(n)],
            PositionFinder.id_().eq(1),
        ).to_pandas()
    ...
    def test_subtract_days_shifts_date_back(self, PositionFinder):
        ...
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().subtract_days(n)],
            PositionFinder.id_().eq(1),
        ).to_pandas()
    ...
    def test_add_months_shifts_date_forward(self, PositionFinder):
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().add_months(1)],
            PositionFinder.id_().eq(1),
        ).to_pandas()
    ...
    def test_add_years_shifts_date_forward(self, PositionFinder):
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().add_years(1)],
            PositionFinder.id_().eq(1),
        ).to_pandas()
    ...
    def test_subtract_months(self, PositionFinder):
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().subtract_months(1)],
            PositionFinder.id_().eq(1),
        ).to_pandas()
    ...
    def test_add_timedelta_operator_matches_add_days(self, PositionFinder):
        ...
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date() + td],
            PositionFinder.id_().eq(1),
        ).to_pandas()
...
class TestDateDiff:
    ...
    def test_diff_days_returns_correct_count(self, PositionFinder):
        ...
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().diff_days(other)],
            PositionFinder.id_().eq(1),
        ).to_pandas()
    ...
    def test_diff_months_returns_correct_count(self, PositionFinder):
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().diff_months(datetime.date(2024, 6, 15))],
            PositionFinder.id_().eq(1),
        ).to_pandas()
    ...
    def test_diff_years_returns_correct_count(self, PositionFinder):
        result = PositionFinder.find_all(
            None, None, [PositionFinder.trade_date().diff_years(datetime.date(2026, 3, 15))],
            PositionFinder.id_().eq(1),
        ).to_pandas()
