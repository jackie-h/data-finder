"""
Runs TRADE_NUMERIC_FINDER_SPECS (abs, ceil, floor, sqrt, mod, power, round)
...
from datafinder_examples_tests.finance_numeric_specs import (
    TRADE_NUMERIC_FINDER_SPECS,
)
...
def _seed_db():
    ...
    conn.execute(f"INSERT INTO ref_data.account_master SELECT * FROM read_csv_auto('{str(example_path('finance_accounts_numeric.csv'))}')")
    ...
    conn.execute(f"INSERT INTO trading.trades SELECT * FROM read_csv_auto('{str(example_path('finance_trades_numeric.csv'))}')")
    conn.close()


@pytest.fixture(scope="module")
def trade_numeric_finder():
    ...
    repo = _build_repo()
    mapping = load(str(example_path(TRADE_NUMERIC_FINDER_SPECS.mapping_file)), repo)
...
@pytest.mark.parametrize(
    "expectation", TRADE_NUMERIC_FINDER_SPECS.expectations, ids=lambda e: e.name
)
def test_trade_numeric(expectation, trade_numeric_finder):
    expectation.run(trade_numeric_finder, backend="duckdb")
